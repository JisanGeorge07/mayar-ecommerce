...
import { categoryService, subcategoryService, productTypeService } from '@/services';
...
export default function HeroBannerForm({ initial, onSave, onCancel }: Props) {
  const [form, setForm] = useState({
    ...
    cta_text_en: initial?.cta_text_en || 'Shop now',
    cta_text_ar: initial?.cta_text_ar || 'تسوق الآن',
  ...
  return (
    <div className="space-y-6">
      ...
      <FormSection title="Admin Settings">
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          <FormField label="Banner Name" required>
            <Input value={form.banner_name} onChange={e => update('banner_name', e.target.value)} placeholder="e.g. Women's Style Edit" />
          </FormField>
          ...
          <TabsContent value="en">
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              <FormField label="Label / Eyebrow (EN)" hint="e.g. CURATED SELECTION">
                <Input value={form.label_en} onChange={e => update('label_en', e.target.value)} placeholder="CURATED SELECTION" />
              </FormField>
              <FormField label="Main Title (EN)" required>
                <Input value={form.title_en} onChange={e => update('title_en', e.target.value)} placeholder="Women's Style Edit" />
              </FormField>
            </div>
            <div className="mt-4">
              <FormField label="Description (EN)">
                <Textarea value={form.description_en} onChange={e => update('description_en', e.target.value)} placeholder="Optional description" rows={2} />
              </FormField>
            </div>
            <div className="mt-4">
              <FormField label="CTA Button Text (EN)">
                <Input value={form.cta_text_en} onChange={e => update('cta_text_en', e.target.value)} placeholder="Shop now" />
              </FormField>
            </div>
          </TabsContent>
          <TabsContent value="ar">
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              <FormField label="Label / Eyebrow (AR)" hint="مثال: مجموعة مختارة">
                <Input value={form.label_ar} onChange={e => update('label_ar', e.target.value)} placeholder="مجموعة مختارة" dir="rtl" />
              </FormField>
              <FormField label="Main Title (AR)" required>
                <Input value={form.title_ar} onChange={e => update('title_ar', e.target.value)} placeholder="تشكيلة أزياء النساء" dir="rtl" />
              </FormField>
            </div>
            <div className="mt-4">
              <FormField label="Description (AR)">
                <Textarea value={form.description_ar} onChange={e => update('description_ar', e.target.value)} placeholder="وصف اختياري" rows={2} dir="rtl" />
              </FormField>
            </div>
            <div className="mt-4">
              <FormField label="CTA Button Text (AR)">
                <Input value={form.cta_text_ar} onChange={e => update('cta_text_ar', e.target.value)} placeholder="تسوق الآن" dir="rtl" />
              </FormField>
            </div>
          </TabsContent>
        </Tabs>
        ...
        <div className="grid grid-cols-1 gap-6 sm:grid-cols-2">
          <div className="space-y-4">
            ...
            <FormField label="Current Price (KWD)">
              <Input type="number" step="0.01" value={form.current_price_kwd} onChange={e => update('current_price_kwd', e.target.value)} placeholder="12.900" />
            </FormField>
            <FormField label="Old Price (KWD)">
              <Input type="number" step="0.01" value={form.old_price_kwd} onChange={e => update('old_price_kwd', e.target.value)} placeholder="18.900" />
            </FormField>
          </div>
          <div className="space-y-4">
            <h4 className="text-sm font-semibold text-foreground">INR Pricing</h4>
            <FormField label="Current Price (INR)">
              <Input type="number" step="1" value={form.current_price_inr} onChange={e => update('current_price_inr', e.target.value)} placeholder="3499" />
            </FormField>
            <FormField label="Old Price (INR)">
              <Input type="number" step="1" value={form.old_price_inr} onChange={e => update('old_price_inr', e.target.value)} placeholder="4999" />
            </FormField>
          </div>
        </div>
      </FormSection>
      ...
      <FormSection title="Media">
        ...
        <div className="mt-4">
          <FormField label="Alt Text">
            <Input value={form.alt_text} onChange={e => update('alt_text', e.target.value)} placeholder="Descriptive alt text for the hero image" />
          </FormField>
        </div>
      </FormSection>
      ...
      <div className="flex items-center justify-end gap-3 pt-2">
        ...
        <Button onClick={handleSubmit}>{initial ? 'Update Banner' : 'Create Banner'}</Button>
      </div>
    </div>
  );
}
