...
import { returnsExchangeService, type ReturnsExchangePageData, type ReturnsSection } from '@/services/returnsExchangeService';

export default function ReturnsExchangePage() {
  const [data, setData] = useState<ReturnsExchangePageData | null>(null);
  const [dirty, setDirty] = useState(false);
  const [openSections, setOpenSections] = useState<Record<string, boolean>>({});
  const [seoOpen, setSeoOpen] = useState(false);

  useEffect(() => {
    returnsExchangeService.getData().then((d) => {
      setData(d);
  ...
  const set = useCallback(<K extends keyof ReturnsExchangePageData>(k: K, v: ReturnsExchangePageData[K]) => {
    setData((prev) => prev ? { ...prev, [k]: v } : prev);
    setDirty(true);
  }, []);

  const updateSection = useCallback((id: string, patch: Partial<ReturnsSection>) => {
    setData((prev) => {
      if (!prev) return prev;
  ...
  const save = async (forceStatus?: 'draft' | 'published') => {
    ...
    const saved = await returnsExchangeService.saveData({ ...data, status: finalStatus });
    setData(saved);
    setDirty(false);
    toast.success(finalStatus === 'published' ? 'Returns & Exchange page published' : 'Draft saved successfully');
  };
  ...
  return (
    <AdminLayout>
      <div className="space-y-6 pb-24">
        <div>
          <h1 className="text-2xl font-bold text-foreground">Returns & Exchange Page</h1>
          <p className="text-sm text-muted-foreground">Manage your Returns & Exchange storefront page content</p>
        </div>
