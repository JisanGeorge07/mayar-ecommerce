...
import { shippingService, type ShippingPageData, type ShippingSection } from '@/services/shippingService';
...
export default function ShippingInformationPage() {
  ...
  useEffect(() => {
    shippingService.getData().then((d) => {
      setData(d);
  ...
  const set = useCallback(<K extends keyof ShippingPageData>(k: K, v: ShippingPageData[K]) => {
    setData((prev) => prev ? { ...prev, [k]: v } : prev);
    setDirty(true);
  }, []);

  const updateSection = useCallback((id: string, patch: Partial<ShippingSection>) => {
    setData((prev) => {
      if (!prev) return prev;
  ...
  const save = async (forceStatus?: 'draft' | 'published') => {
    ...
    const saved = await shippingService.saveData({ ...data, status: finalStatus });
    setData(saved);
    setDirty(false);
    toast.success(finalStatus === 'published' ? 'Shipping Information page published' : 'Draft saved successfully');
  };
  ...
  return (
    <AdminLayout>
      <div className="space-y-6 pb-24">
        <div>
          <h1 className="text-2xl font-bold text-foreground">Shipping Information Page</h1>
          <p className="text-sm text-muted-foreground">Manage your Shipping Information storefront page content</p>
        </div>
